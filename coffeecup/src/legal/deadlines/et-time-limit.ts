...
/**
 * Result of a single ET time-limit calculation. Retained from Tribunal Harness
 * (see docs/MIGRATION_FROM_TRIBUNAL_HARNESS.md); the versioned rule registry
 * in src/legal/rules wraps this into the user-facing DeadlineExplanation.
 */
export interface DeadlineResult {
    claim_type: string;
    /** Statutory deadline BEFORE any Acas Early Conciliation extension. */
    base_deadline: string;
    /** Deadline AFTER the Acas clock-stop, present only when Day A / Day B were supplied. */
    acas_extended_deadline?: string;
    /** The operative deadline the claimant must actually meet. */
    final_deadline: string;
    /** @deprecated alias of final_deadline kept for the retained tests. */
