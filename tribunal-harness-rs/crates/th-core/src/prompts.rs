//! Claude Agent Prompts — port of `src/agents/prompts.ts`, byte-for-byte
//! (verified against `fixtures/prompts/prompts.json`).
//!
//! Design principles (from the TypeScript source):
...
use crate::constants::{era_2025_get, format_commencement_label, is_commencement_tbc, ERA_2025};
use std::sync::LazyLock;

/// Prompt version constants — logged with every API call for the audit trail.
pub mod versions {
    pub const ANALYSE: &str = "ANALYSE_PROMPT_v2";
    pub const TRIAGE: &str = "TRIAGE_PROMPT_v2";
    pub const DRAFTER: &str = "DRAFTER_PROMPT_v2";
    pub const CRITIC: &str = "CRITIC_PROMPT_v2";
    pub const JUDGE: &str = "JUDGE_PROMPT_v2";
    pub const REFINEMENT: &str = "v1";
}

pub const REFINEMENT_PROMPT_VERSION: &str = "v1";

/// F-16 (Hard Rule 6): render a commencement date for prompt injection. A
/// TBC date gets an explicit "(exact date TBC by SI)" marker; a null key
/// renders as "a date to be confirmed by Statutory Instrument".
fn commencement(key: &str) -> String {
    match era_2025_get(key) {
        Some(Some(iso)) => format_commencement_label(iso, is_commencement_tbc(key)),
        _ => "a date to be confirmed by Statutory Instrument".to_string(),
    }
}

/// ANALYSE_PROMPT_v2 — anti-hallucination, confidence bands, structured JSON.
pub static ANALYSE_PROMPT_V2: LazyLock<String> = LazyLock::new(|| {
    format!(
        r#"You are a UK employment law analysis engine providing legal INFORMATION — NOT legal advice. You operate under the Legal Services Act 2007 regulatory boundary.
...
Royal Assent: {royal_assent}. You MUST check whether ERA 2025 provisions apply to the dates provided:
- EDT on/after {qp}: qualifying period for unfair dismissal is 6 months (not 2 years)
- EDT on/after {qp}: compensatory award cap is removed entirely
- EDT on/after {far}: fire-and-rehire dismissals are automatically unfair
- Act date on/after {tl}: ET time limit is 6 months less 1 day (not 3 months less 1 day)
- Act date on/after {hars}: employer harassment duty is "all reasonable steps" (not "reasonable steps")
- Act date on/after {tph}: third-party harassment liability applies
- Act date on/after {nda}: NDAs for harassment/discrimination are void
- From {shw}: sexual harassment is a qualifying disclosure for whistleblowing (Part IVA ERA 1996)
- From {iad}: industrial action dismissal is automatically unfair (12-week limit removed)
...
{{
  ...
  "era_2025_flags": [{{ "provision": string, "applies": boolean, "reason": string, "commencement_date": string, "status": "in_force"|"upcoming"|"tbc" }}]
        ...
        royal_assent = ERA_2025.royal_assent,
        qp = ERA_2025.qualifying_period_6_months,
        far = ERA_2025.fire_and_rehire_auto_unfair,
        tl = commencement("ET_TIME_LIMIT_6_MONTHS"),
        hars = commencement("HARASSMENT_ALL_REASONABLE_STEPS"),
        tph = commencement("THIRD_PARTY_HARASSMENT"),
        nda = commencement("NDA_VOID"),
        shw = ERA_2025.sexual_harassment_whistleblowing,
        iad = ERA_2025.industrial_action_dismissal,
    )
});

/// TRIAGE_PROMPT_v2 — structured extraction, date heuristics.
pub const TRIAGE_PROMPT_V2: &str = r#"You are a UK employment law triage agent. You extract structured information from documents to populate claim schemas.
...
pub static DRAFTER_PROMPT_V2: LazyLock<String> = LazyLock::new(|| {
    format!(
        r#"You are the DRAFTER agent in an adversarial debate regarding a UK employment tribunal claim (England & Wales jurisdiction).
...
- Qualifying period for unfair dismissal: 6 months from {qp} (currently 2 years)
- ET time limit: 6 months less 1 day from {tl} (currently 3 months less 1 day)
- Fire and rehire: automatically unfair from {far}
- Harassment duty: "all reasonable steps" from {hars}
...
pub const CRITIC_PROMPT_V2: &str = r#"You are the CRITIC agent in an adversarial debate regarding a UK employment tribunal claim (England & Wales jurisdiction).
...
pub const JUDGE_PROMPT_V2: &str = r#"You are the JUDGE agent in an adversarial debate regarding a UK employment tribunal claim (England & Wales jurisdiction).
...
#[deprecated(note = "Use ANALYSE_PROMPT_V2")]
pub static ANALYSE_PROMPT_V1: LazyLock<String> = LazyLock::new(|| {
    format!(
        r#"You are a UK employment law analysis engine. You provide legal information — NOT legal advice.
...
CRITICAL — ERA 2025 (Employment Rights Act 2025):
The ERA 2025 received Royal Assent on {royal_assent}. You MUST check whether ERA 2025 provisions apply:
- If EDT is on/after {qp}: qualifying period for unfair dismissal is 6 months (not 2 years)
- If EDT is on/after {qp}: compensatory award cap is removed
- If EDT is on/after {far}: fire-and-rehire dismissals are automatically unfair
- If act date is on/after {tl}: time limit is 6 months less 1 day (not 3 months)
- If act date is on/after {hars}: harassment duty is "all reasonable steps" (not "reasonable steps")
- If act date is on/after {tph}: third-party harassment liability applies
- If act date is on/after {nda}: NDAs for harassment/discrimination are void
- From {shw}: sexual harassment is a qualifying disclosure for whistleblowing
- From {iad}: industrial action dismissal is automatically unfair (no 12-week limit)
...
Respond in structured JSON matching the AnalyseResponse interface."#,
        ...
        qp = ERA_2025.qualifying_period_6_months,
        far = ERA_2025.fire_and_rehire_auto_unfair,
        tl = ERA_2025.et_time_limit_6_months,
        hars = ERA_2025.harassment_all_reasonable_steps,
        tph = ERA_2025.third_party_harassment,
        nda = ERA_2025.nda_void,
        shw = ERA_2025.sexual_harassment_whistleblowing,
        iad = ERA_2025.industrial_action_dismissal,
    )
});

#[deprecated(note = "Use TRIAGE_PROMPT_V2")]
pub const TRIAGE_PROMPT_V1: &str = r#"You are a UK employment law triage agent.
...
#[deprecated(note = "Use DRAFTER_PROMPT_V2")]
pub const DRAFTER_PROMPT_V1: &str = r#"You are the Drafter agent in an adversarial debate regarding a UK employment tribunal claim.
...
#[deprecated(note = "Use CRITIC_PROMPT_V2")]
pub const CRITIC_PROMPT_V1: &str = r#"You are the Critic agent in an adversarial debate regarding a UK employment tribunal claim.
...
#[deprecated(note = "Use JUDGE_PROMPT_V2")]
pub const JUDGE_PROMPT_V1: &str = r#"You are the Judge agent in an adversarial debate regarding a UK employment tribunal claim.
...
/// LEGAL_WRITING_REFINEMENT_PROMPT_v1 — editor-only pass over allowlisted prose.
pub const LEGAL_WRITING_REFINEMENT_PROMPT_V1: &str = r#"You are a senior UK employment-law editor refining LLM-generated prose
...
mod tests {
    ...
    #[test]
    fn tbc_dates_carry_marker_and_never_leak_bare_iso() {
        let marker = "(exact date TBC by SI)";
        assert!(ANALYSE_PROMPT_V2.contains(marker));
        assert!(DRAFTER_PROMPT_V2.contains(marker));
        for key in ["ET_TIME_LIMIT_6_MONTHS", "HARASSMENT_ALL_REASONABLE_STEPS", "THIRD_PARTY_HARASSMENT", "NDA_VOID"] {
            let iso = era_2025_get(key).unwrap().unwrap();
            assert!(!ANALYSE_PROMPT_V2.contains(iso));
            assert!(!DRAFTER_PROMPT_V2.contains(iso));
        }
        assert!(ANALYSE_PROMPT_V2.contains("October 2026 (exact date TBC by SI)"));
        assert!(DRAFTER_PROMPT_V2.contains(ERA_2025.fire_and_rehire_auto_unfair));
    }
}
