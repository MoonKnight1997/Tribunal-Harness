...
pub fn schema() -> ClaimSchema {
    ClaimSchema {
        ...
        era2025_changes: Some(vec!["Entirely new set of rights created by ERA 2025 (from 2027)".into(), "Rights extend to agency workers".into()]),
        fields: vec![
            SchemaField::new("contract_type", "Contract Type", FieldType::Select, true)
                .options(vec![opt("zero_hours", "Zero-hours contract"), opt("low_hours", "Low-hours contract"), opt("agency", "Agency worker")])
            ...
            SchemaField::new("cancellation_payment", "Was Cancellation Payment Made?", FieldType::Boolean, false).help("Payment for shifts cancelled at short notice."),
