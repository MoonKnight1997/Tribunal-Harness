...
const USER_AGENT = "coffeecup/0.1 (UK workplace-dispute support tool; legal information only)";
...
export interface JudgmentMarkdown {
    slug: string;
    status: import("@/documents/extract/pdf").PdfStatus;
    markdown?: string;
    pages?: number;
    sourceUrl: string;
    detail?: string;
...
export async function getJudgmentMarkdown(slug: string): Promise<JudgmentMarkdown> {
    ...
    const { fetchPdfAsMarkdown } = await import("@/documents/extract/pdf");
