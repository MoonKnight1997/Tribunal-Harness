...
`tribunal-harness-rs/` is a Rust (Axum + `maud`) port of the earlier Tribunal
Harness Next.js app, with parity established by recorded fixture-diff tests. The
TypeScript app it was ported from has since been replaced by coffeecup, so the
Rust port is kept as a standalone reference build. See
[`tribunal-harness-rs/README.md`](tribunal-harness-rs/README.md) and
[`tribunal-harness-rs/PARITY.md`](tribunal-harness-rs/PARITY.md).

```bash
cd tribunal-harness-rs
cargo test --workspace                     # hermetic, no network, no API key
LLM_PROVIDER=agent cargo run --bin smoke   # offline end-to-end smoke run
```
